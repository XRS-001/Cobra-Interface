...
class AuthServiceProxy(object):
    ...
    def __init__(self, service_url, service_name=None, timeout=HTTP_TIMEOUT, 
        ...
        try:
            user = user.encode('utf8') # type: ignore
        except AttributeError:
            pass
        try:
            passwd = passwd.encode('utf8') # type: ignore
        except AttributeError:
            pass
        authpair = user + b':' + passwd # type: ignore
        ...
        if connection:
            ...
            self.__conn = httplib.HTTPSConnection(self.__url.hostname, port, # type: ignore
                                                  timeout=timeout, context=ssl_context)
        else:
            self.__conn = httplib.HTTPConnection(self.__url.hostname, port, # type: ignore
                                                 timeout=timeout)
    ...
    def __call__(self, *args):
        ...
        self.__conn.request('POST', self.__url.path, postdata,
                            {'Host': self.__url.hostname, # type: ignore
    ...
    def _get_response(self):
        ...
        if not content_type.startswith('application/json'): # type: ignore
            raise JSONRPCException({
